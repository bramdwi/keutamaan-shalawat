  ...
  return (
    <div ref={readerTopRef} className="max-w-4xl mx-auto px-3.5 sm:px-6 py-6 sm:py-10">
      ...
      <div className="mb-8 sm:mb-12 text-center relative overflow-hidden rounded-2xl sm:rounded-3xl p-6 sm:p-10 border border-emerald-600/20 bg-gradient-to-b from-emerald-700/10 via-emerald-600/5 to-transparent">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-emerald-600/15 text-emerald-800 dark:text-emerald-300 mb-3 border border-emerald-600/20">
        ...
        {chapter.subtitle && (
          <p className="text-sm sm:text-base font-medium opacity-80 max-w-2xl mx-auto mb-4">
            {chapter.subtitle}
          </p>
        )}

        {chapter.description && (
          <p className="text-xs sm:text-sm opacity-70 max-w-xl mx-auto leading-relaxed italic border-t border-black/5 dark:border-white/10 pt-3">
            "{chapter.description}"
          </p>
        )}
      </div>
      ...
      <div className="space-y-6 sm:space-y-8">
        {chapter.sections.length === 0 ? (
          <div className="text-center py-16 px-4 rounded-2xl border border-dashed border-emerald-600/30 bg-emerald-500/5">
            ...
            <h3 className="text-lg font-bold mb-1">Menunggu Isi Teks Bab Ini</h3>
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 pb-4 border-b border-black/10 dark:border-white/10 mb-6">
                  <div className="flex items-start gap-2.5">
                    <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-emerald-600/15 text-emerald-800 dark:text-emerald-300 font-bold text-xs sm:text-sm flex items-center justify-center shrink-0 mt-0.5 border border-emerald-600/20">
                    ...
                    <div>
                      ...
                      {section.hadithNumber && (
                        <div className="flex items-center gap-1 text-[11px] sm:text-xs opacity-70 mt-1">
                ...
                {preferences.showExplanation &&
                  ...
                  section.explanation.length > 0 && (
                    <div className={`mt-6 p-4 sm:p-5 rounded-xl sm:rounded-2xl border ${getFaedahBoxBg()}`}>
                      <div className="flex items-center gap-1.5 font-bold text-xs sm:text-sm mb-2.5 text-emerald-800 dark:text-emerald-300 uppercase tracking-wide">
                ...
                {section.tags && section.tags.length > 0 && (
                  <div className="mt-4 pt-3 border-t border-black/5 dark:border-white/5 flex flex-wrap gap-1.5">
                    {section.tags.map((tag, tIdx) => (
                      <span
                        key={tIdx}
                        className="text-[10px] sm:text-[11px] px-2 py-0.5 rounded-md bg-black/5 dark:bg-white/10 opacity-70 font-medium"
                      >
                        #{tag}
                      </span>
                    ))}
